...
| **UI 骨架** | 三栏 Workspace（左侧边栏 / 编辑区 / 右侧边栏）；resizable panels（v4 API）；ThemeProvider（亮/暗/护眼）；CommandPalette / SettingsDialog / SearchDialog 框架；Workspace shell 已接入语义图标（左右 Tab、Footer 字数区、TitleBar 面包屑）；章节树、搜索弹窗、命令面板完成第一轮视觉精修 |

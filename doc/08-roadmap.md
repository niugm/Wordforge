...
| **F6 AI 精修** | Rust 侧新增 `ai` 模块与 `LlmProvider` 抽象雏形；OpenAI-compatible provider 已支持 `/chat/completions` 非流式请求；新增 `ai_polish` IPC，支持凝练 / 扩写 / 增强描写 / 调整语气 / 自由指令；右侧 AI 面板已从占位改为可手动粘贴文本的精修入口，支持结果复制与重试；超过 5000 字会阻止调用；当前项目可用时写入 `ai_messages.scope = 'polish'` |
...
| 优先级 | 功能 | 说明 |
|---|---|---|
| P0 | F6 AI 精修编辑器接入 | 非流式调用链路和右侧手动入口已跑通；下一步把 TipTap 选区/当前段落自动送入 AI 面板，并实现替换/插入时写 `revisions.source = 'ai_polish'` |

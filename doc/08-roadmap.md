...
| **F6 AI 精修** | Rust 侧新增 `ai` 模块与 `LlmProvider` 抽象雏形；OpenAI-compatible provider 已支持 `/chat/completions` 非流式请求；新增 `ai_polish` IPC，支持凝练 / 扩写 / 增强描写 / 调整语气 / 自由指令；右侧 AI 面板已从占位改为精修入口，支持结果复制、重试、替换原文、插入下方和原文/建议轻量对照；编辑器选区 / 当前段落可通过 BubbleMenu `AI` 或 Ctrl/Cmd+J 送入 AI 面板；超过 5000 字会阻止调用；当前项目可用时写入 `ai_messages.scope = 'polish'`；替换前写入 `revisions.source = 'ai_polish'` |
...
| 优先级 | 功能 | 说明 |
|---|---|---|
| P0 | F6 AI 精修深化 | 编辑器选区/当前段落接入、替换/插入和轻量对照已跑通；下一步补选区 popover 动作菜单和 streaming |

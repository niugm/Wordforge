...
| v0.0.7 | F10 全文检索 + F9 批注基础 | 🚧 进行中 | `5bda4d9` |
| v0.0.8 | F6 AI 精修 + F7 章节校审 | 🚧 进行中 | |
...
| **F6 AI 精修** | Rust 侧新增 `ai` 模块与 `LlmProvider` 抽象雏形；OpenAI-compatible provider 已支持 `/chat/completions` 非流式 / 流式请求；新增 `ai_polish` / `ai_polish_stream` / `cancel_ai_polish_stream` IPC，支持凝练 / 扩写 / 增强描写 / 调整语气 / 自由指令；右侧 AI 面板已从占位改为精修入口，支持流式输出、停止 / 继续生成、结果复制、重试、替换原文、插入下方和原文/建议 token 级对照；编辑器选区 / 当前段落可通过 BubbleMenu `AI` 动作菜单或 Ctrl/Cmd+J 送入 AI 面板；超过 5000 字会阻止调用；当前项目可用时写入 `ai_messages.scope = 'polish'`；替换前写入 `revisions.source = 'ai_polish'` |
| **F7 章节校审** | 右侧 AI 面板新增精修 / 校审模式切换；新增 `ai_review_chapter` IPC，读取当前章节正文并要求模型输出结构化 JSON；自动带入当前作品角色卡与大纲摘要；按逻辑 / 连贯 / 人物口吻 / 伏笔分组标签渲染建议卡；报告写入 `ai_messages.scope = 'review'`；每条建议可复制；支持按严重程度筛选、忽略 / 恢复，忽略状态按章节持久化，并可基于原文短引文定位到正文；支持把建议送入精修生成候选改写；可一键采纳模型给出的局部改写并写入修订记录 |
...
| 优先级 | 功能 | 说明 |
|---|---|---|
| P0 | F7 章节校审深化 | 提升跨段落定位鲁棒性、支持手动选择校审上下文 |
...
| P1 | F13 导出深化 | Docx 导出 |
| P1 | F6 AI 上下文包深化 | 可选角色卡 / 大纲节点手动选择、带入段落精修 |
...
| 风险 | 概率 | 影响 | 缓解 |
|---|---|---|---|
| Tauri 2 plugin-stronghold 在 Windows 编译困难 | 中 | 中 | v0.1 已改用 keyring crate（OS Credential Manager），Stronghold 留作 v0.2 主密码方案 |

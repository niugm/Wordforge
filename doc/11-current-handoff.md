...
- 每条校审建议支持忽略 / 恢复；面板可切换显示已忽略建议；忽略状态按章节写入 `wordforge-ui` 持久化。
- 校审建议卡显示原文短引文，便于作者判断定位依据。
- 校审 JSON schema 新增 `replacementText` 字段，模型可给出能直接替换短引文的局部改写。
- 每条建议新增“采纳建议”入口：
  - 先按原文短引文定位正文范围。
  - 替换前写入 `revisions.source = 'review'`。
  - 仅替换短引文对应范围，不直接改动章节其他正文。
- 每条校审建议新增“按建议改写”入口：
  - 先通过原文短引文定位并选中正文。
  - 再把选区送入 AI 精修自由指令模式。
  - 自由指令自动带入问题类型、严重程度、问题说明和修改建议。
  - 最终仍复用现有 `替换原文` / `插入下方` 确认链路，不直接覆盖正文。
- `ai_review_chapter` 会自动构造上下文包：
  - 最多带入 12 个角色卡摘要（姓名、别名、身份、画像截断）。
  - 最多带入 16 个大纲节点摘要（标题、状态、内容截断）。
  - prompt 会要求模型优先用这些上下文判断人物口吻、设定一致性与伏笔矛盾。
...
- AI 章节校审已接入当前章节全文发送、角色卡 / 大纲摘要上下文包、结构化 JSON 解析、建议卡片渲染、`ai_messages.scope = 'review'` 留痕、严重程度筛选、忽略 / 恢复、忽略状态按章节持久化、基于原文短引文定位、按建议生成候选改写，以及一键采纳局部改写。
- 校审上下文包目前自动截断带入，暂不支持手动勾选角色卡或大纲节点；跨段落定位仍是基于短引文的基础版。
...
1. 深化章节校审：提升跨段落定位鲁棒性、支持手动选择校审上下文。
2. 首次 GitHub Actions 实跑验证。
3. 继续深化 AI 上下文包：可选角色卡 / 大纲节点手动选择，并带入段落精修。
